=== SNEFERU Geo WordPress Connector ===
Contributors: tdsgeo
...
Thin connector for SNEFERU Geo Core - exposes WordPress REST API for post CRUD, media, taxonomies, and settings. All AI intelligence runs on the Core backend.

== Description ==

SNEFERU Geo WordPress Connector is a lightweight REST API bridge between your WordPress site and SNEFERU Geo Core (the central AI engine). After installation and API connection, SNEFERU Geo Core can create, update, delete, and sync content on your WordPress site through this connector.
...
* **Webhook Support** — Signature-verified incoming webhooks from SNEFERU Geo Core
...
* ❌ No content generation — all AI writing runs on SNEFERU Geo Core
* ❌ No SEO analysis — all scoring runs on SNEFERU Geo Core
* ❌ No research — all keyword research runs on SNEFERU Geo Core
* ❌ No memory or knowledge base — that lives on SNEFERU Geo Core
* ❌ No self-healing — that's managed by SNEFERU Geo Core

== Installation ==

1. Upload the `tds-geo-wp` folder to the `/wp-content/plugins/` directory, or upload the ZIP directly.
2. Activate the plugin through the 'Plugins' screen in WordPress.
3. Go to SNEFERU Geo Connector → API Keys to generate an API key.
4. Enter the SNEFERU Geo Core URL and API key in the Settings page.
5. The connector will register with SNEFERU Geo Core and begin accepting commands.

== Frequently Asked Questions ==

= Does this plugin generate content? =

No. All content generation, SEO analysis, and research runs on SNEFERU Geo Core. This plugin only provides the REST API endpoints for SNEFERU Geo Core to manage content on your WordPress site.
...
= How do I connect this to SNEFERU Geo Core? =

Go to SNEFERU Geo Connector → API Keys, generate a new API key, then enter the SNEFERU Geo Core URL and your API key in the Settings page. SNEFERU Geo Core will then be able to communicate with your site.

== Changelog ==

= 3.0.0 =
* Complete rewrite as thin connector — all AI logic moved to SNEFERU Geo Core
...
* Added payload normalization for newer SNEFERU Geo backend field names (`contentHtml`, `metaTitle`, `metaDescription`, `imageUrl`, `publishedAt`, and related camelCase keys)
