...
(function($) {
  ...
  function loadDashboard() {
    ...
    setLoading(true);
    $.post(tds.ajaxUrl, { action: 'kivo_dashboard_data', nonce: tds.nonce }, function(r) {
  ...
  function testConnection() {
    ...
    if (!tds.ajaxUrl || !$btn.length || $btn.hasClass('tds-loading')) return;

    $btn.addClass('tds-loading').prop('disabled', true).text('Testing...');
    $('.tds-test-result').remove();

    $.post(tds.ajaxUrl, { action: 'kivo_test_connection', nonce: tds.nonce, _ajax_nonce: tds.nonce }, function(r) {
      var ok = !!(r && r.success);
      var message = ok
        ? ('Connected to ' + (r.data?.site_name || 'site') + ' • SEO plugins: ' + (r.data?.seo_plugins ?? 0))
        : (r.data?.message || 'Connection test failed.');
      var cls = ok ? 'tds-test-success' : 'tds-test-fail';
      $('<div class="tds-test-result ' + cls + '">').text(message).insertAfter($btn);
      showToast(message, ok ? 'success' : 'error');
    }).fail(function(xhr) {
      var message = 'Server error: ' + (xhr.statusText || 'unknown');
      $('<div class="tds-test-result tds-test-fail">').text(message).insertAfter($btn);
      showToast(message, 'error');
    }).always(function() {
      $btn.removeClass('tds-loading').prop('disabled', false).text('Test Connection');
    });
  }
  ...
  window.revealApiKey = function(keyId) {
    ...
    $.post(tdsGeo?.ajax_url || ajaxurl, {
      action: 'kivo_reveal_key', key_id: keyId, password: password, nonce: tdsGeo?.nonce || '', _ajax_nonce: tdsGeo?.nonce || ''
    }, function(r) {
  ...
  $(function() {
    ...
    $(document).on('click', '#tds-test-api', function(e) { e.preventDefault(); testConnection(); });
    $(document).on('click', '.tds-milestone-close', function() { dismissMilestone(); });

    if ($('.tds-admin-dashboard').length) { loadDashboard(); scheduleRefresh(); }
  });
})(jQuery);
