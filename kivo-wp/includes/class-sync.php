...
class Sync {
    ...
    public static function create_post(array $article): array {
        ...
        $agent_id = $article['agent_article_id'] ?? $article['kivo_article_id'] ?? '';
        update_post_meta($post_id, '_kivo_article_id', $agent_id);
        update_post_meta($post_id, '_kivo_imported_at', current_time('mysql'));

        global $wpdb;
        $wpdb->replace($wpdb->prefix . 'kivo_articles', [
            'post_id'          => $post_id,
    ...
    public static function update_post(int $post_id, array $article): array {
        ...
        update_post_meta($post_id, '_kivo_last_updated', current_time('mysql'));
    ...
    private static function build_post_data(array $article, ?\WP_Post $existing = null): array {
        $settings = get_option('kivo_wp_settings', []);
        $kivo_status = strtolower($article['status'] ?? 'draft');
        $wp_status = self::STATUS_MAP[$kivo_status] ?? 'draft';
    ...
    private static function handle_meta(int $post_id, array $article): void {
        ...
        // Always store
        if ($mt) update_post_meta($post_id, '_kivo_meta_title', $mt);
        if ($md) update_post_meta($post_id, '_kivo_meta_description', $md);
        if ($kw) update_post_meta($post_id, '_kivo_focus_keyword', $kw);

        if (!empty($article['schema'])) {
            update_post_meta($post_id, '_kivo_schema', $article['schema']);
        }
    }

    private static function set_custom_fields(int $post_id, array $custom_fields): void {
        foreach ($custom_fields as $key => $value) {
            if (str_starts_with($key, '_kivo_')) continue;
    ...
    public static function get_agent_posts(int $limit = 50, string $status = ''): array {
        ...
        foreach ($posts as $post) {
            $results[] = [
                ...
                'agent_article_id'  => get_post_meta($post->ID, '_kivo_article_id', true),
