...
class Activator {
    ...
    public static function activate(): void {
        ...
        try {
            ...
            $errors[] = 'Migration from kivo-wp failed: ' . $e->getMessage();
        }

        if (!empty($errors)) {
            set_transient('kivo_wp_activation_notice', $errors, 30);
        }
    }
    ...
    private static function set_defaults(): void {
        ...
        $existing = get_option('kivo_wp_settings', null);
        if (!is_array($existing)) {
            add_option('kivo_wp_settings', $defaults, '', 'yes');
        }

        if (!get_option('kivo_wp_initial_key')) {
            $api_key = 'kai_' . bin2hex(kivo_wp_random_bytes(24));
            add_option('kivo_wp_initial_key', $api_key, '', 'no');
    ...
    private static function migrate_from_core_wp(): void {
        ...
        $core_log_table = $wpdb->prefix . 'kivo_logs';
        $core_keys_table = $wpdb->prefix . 'kivo_api_keys';
        $has_core_tables = $wpdb->get_var("SHOW TABLES LIKE '{$core_log_table}'");

        if (empty($has_core_tables)) return;

        // Migrate API keys: mark existing kivo keys, copy core keys
        $ai_keys_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}kivo_api_keys");
        if ($ai_keys_count === 0) {
            $wpdb->query(
                "INSERT IGNORE INTO {$wpdb->prefix}kivo_api_keys
        ...
        $core_settings = get_option('kivo_settings', null);
        if (is_array($core_settings)) {
            $ai_settings = get_option('kivo_wp_settings', []);
            ...
            update_option('kivo_wp_settings', $ai_settings);
        }

        // Migrate initial key
        $core_initial = get_option('kivo_initial_api_key', '');
        if (!empty($core_initial) && !get_option('kivo_wp_initial_key', '')) {
            add_option('kivo_wp_initial_key', $core_initial, '', 'no');
        }
    }
}
