...
class Auth {
    ...
    public static function init(): void {
        ...
        self::ensure_schema();
        add_action('admin_post_kivo_generate_key', [self::class, 'handle_generate_key']);
        add_action('admin_post_kivo_revoke_key', [self::class, 'handle_revoke_key']);
        add_action('wp_ajax_kivo_reveal_key', [self::class, 'handle_reveal_key']);
    }

    private static function table_exists(): bool {
        global $wpdb;
        $table = $wpdb->prefix . 'kivo_api_keys';
    ...
    private static function get_encryption_key(): string {
        ...
        _doing_it_wrong(__FUNCTION__, 'Kivo Geo: No WordPress secret key found. Set NONCE_KEY in wp-config.php for secure API key encryption.', '2.0.0');
        return wp_hash('kivo-encryption-key-' . DB_NAME);
    }
    ...
    private static function mask_key(?string $api_key): string {
        ...
        return __('Stored securely', 'kivo-wp');
    }
    ...
    public static function generate_key(string $label = '', string $permissions = 'read,write', int $created_by = 0, int $expires_in = 0): array {
        ...
        if (!self::table_exists()) {
            return ['success' => false, 'message' => __('Database tables are missing. Please deactivate and reactivate the plugin.', 'kivo-wp')];
        }
        self::ensure_schema();
        ...
        if (!$inserted) {
            $db_error = $wpdb->last_error ?: __('Unknown database error.', 'kivo-wp');
            Logger::error('API key generation failed', ['service' => 'auth', 'db_error' => $db_error]);
            return ['success' => false, 'message' => sprintf(__('Failed to generate API key: %s', 'kivo-wp'), $db_error)];
        }

        Logger::info('API key generated', ['label' => $label, 'permissions' => $permissions]);
        return ['success' => true, 'api_key' => $api_key, 'message' => __('API key generated.', 'kivo-wp')];
    }
    ...
    public static function validate_key(string $api_key): array {
        ...
        if (null === self::$active_keys) {
            ...
            $results = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT id, api_key, api_key_hash, permissions FROM {$wpdb->prefix}kivo_api_keys
    ...
    public static function list_keys(): array {
        ...
        $keys = $wpdb->get_results(
            "SELECT id, api_key, label, permissions, is_active, last_used_at, expires_at, created_at
             FROM {$wpdb->prefix}kivo_api_keys ORDER BY created_at DESC",
            ARRAY_A
        );
    ...
    public static function reveal_key(int $key_id): ?string {
        ...
        $row = $wpdb->get_row($wpdb->prepare(
            "SELECT api_key, api_key_encrypted FROM {$wpdb->prefix}kivo_api_keys WHERE id = %d",
            $key_id
        ), ARRAY_A);
        if (!$row) return null;
    ...
    public static function handle_reveal_key(): void {
        ...
        if (!wp_verify_nonce($_POST['nonce'] ?? ($_POST['_ajax_nonce'] ?? ''), 'kivo_wp_ajax')) {
    ...
    public static function authenticate_request(): array {
        ...
        if (!empty($_SERVER['HTTP_X_KIVO_KEY'])) {
            $api_key = sanitize_text_field(wp_unslash($_SERVER['HTTP_X_KIVO_KEY']));
        }
        ...
        if (empty($api_key)) {
            return ['valid' => false, 'permissions' => [], 'message' => 'Missing API key. Provide it via X-Kivo-Key or Authorization: Bearer.'];
        }
    ...
    public static function handle_generate_key(): void {
        ...
        check_admin_referer('kivo_generate_key', 'kivo_generate_nonce');
        ...
        $redirect = admin_url('admin.php?page=kivo-wp-keys');
        if ($result['success']) {
            set_transient('kivo_wp_new_key_' . get_current_user_id(), $result['api_key'], MINUTE_IN_SECONDS * 10);
            $redirect = add_query_arg('generated', '1', $redirect);
        } else {
            set_transient('kivo_wp_key_error_' . get_current_user_id(), $result['message'] ?? __('Failed to generate API key.', 'kivo-wp'), MINUTE_IN_SECONDS * 10);
    ...
    public static function handle_revoke_key(): void {
        if (!current_user_can('manage_options')) wp_die('Unauthorized');
        check_admin_referer('kivo_revoke_key', 'kivo_revoke_nonce');

        $key_id = absint(wp_unslash($_POST['key_id'] ?? 0));
        if ($key_id > 0) self::revoke_key_by_id($key_id);

        wp_safe_redirect(admin_url('admin.php?page=kivo-wp-keys&updated=1'));
        exit;
    }
}
