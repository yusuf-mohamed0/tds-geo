...
/**
 * REST API endpoints - thin connector for Kivo Geo.
 * Registers both kivo/v1 and (for backward compat) kivo/v1.
 */
class Api {
    ...
    public static function register_routes(): void {
        self::register_route_set(KIVO_WP_API_NAMESPACE);
    }
    ...
    private static function rate_limit_response(string $scope, int $max_requests, int $window): \WP_Error|true {
        ...
        return new \WP_Error(
            'kivo_rate_limited',
            __('Rate limit exceeded.', 'kivo-wp'),
    ...
    public static function get_status(): \WP_REST_Response {
        ...
        $imported_posts = (int) $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = '_kivo_imported_at'"
        );
        ...
        return new \WP_REST_Response([
            ...
            'data'    => [
                ...
                'locale'           => get_locale(),
                'api_namespace'    => KIVO_WP_API_NAMESPACE,
                'api_base_url'     => rest_url(KIVO_WP_API_NAMESPACE),
                'webhook_url'      => rest_url(KIVO_WP_API_NAMESPACE . '/webhook'),
                'connector_version'=> KIVO_CONNECTOR_VERSION,
    ...
    public static function list_posts(\WP_REST_Request $request): \WP_REST_Response {
        ...
        if ($agent_id) {
            $args['meta_key'] = '_kivo_article_id';
    ...
    public static function get_settings(): \WP_REST_Response {
        ...
        $settings['version'] = KIVO_WP_VERSION;
    ...
    public static function handle_webhook(\WP_REST_Request $request): \WP_REST_Response {
        ...
        if (!empty($settings['webhook_secret'])) {
            $signature = $request->get_header('X-KIVO-Signature') ?: $request->get_header('X-KIVO-Signature-256');
        ...
        switch ($event) {
            ...
            case 'article.updated':
                ...
                $agent_id = $data['agent_article_id'] ?? $data['kivo_article_id'] ?? '';
    ...
    private static function format_post(\WP_Post $post): array {
        ...
        return [
            ...
            'meta'              => ['title' => get_post_meta($post->ID, '_kivo_meta_title', true), 'description' => get_post_meta($post->ID, '_kivo_meta_description', true)],
            'agent_id'          => get_post_meta($post->ID, '_kivo_article_id', true),
            'imported_at'       => get_post_meta($post->ID, '_kivo_imported_at', true),
