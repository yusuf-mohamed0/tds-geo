...
| --- | --- | --- | --- |
| Shopify | Integration Manager | `shopifyInstall` OAuth writes tokens encrypted into `clients`; `refreshIfExpired` decrypts on read, refreshes via OAuth, re-encrypts on write, and audits to `credential_access_log` (`refresh`); boot hardening encrypts legacy rows and maintains masked vault policy rows (rotation_days=30); live publish only via explicit `publish:true`, approved/scheduled rows, or `/publish-all` over `approved`. | Hidden draft, manual approval, and scheduled publish semantics remain intact; no raw token JSON in `cms_connections.config`; next hardening step is an on-demand OAuth-refresh rotate endpoint and scheduled rotation-policy check. |
...
The next implementation phase should add an on-demand Shopify OAuth-refresh rotate endpoint (`POST /api/shopify/rotate`) and a scheduled rotation-policy check flagging tokens older than `rotation_days`. Until then, all integration surfaces stay on their current behaviors.