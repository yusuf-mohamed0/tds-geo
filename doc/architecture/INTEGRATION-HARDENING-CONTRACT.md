...
| Nextcloud / backups | Storage Manager | `sync-nextcloud-safe.sh` stages docs/outputs/reports and deletes `.env`, `*.pem`, `*.key`, `*token*`, `clients.yaml`, and vault paths before WebDAV upload; `backup-db.sh` runs `pg_dump` and optional S3 upload. Sync mechanics were verified end-to-end 2026-08-17 against the live instance (PROPFIND 207, MKCOL 201, `sync_ok`). | The artifact boundary is not yet met: credential-bearing client doc files (`doc/clients/*/technical-reference.md`, `contacts.md`, `full-profile.md`, `brand-profile.md`) are staged wholesale and the exclusion list does not cover them. Next hardening step is extending the exclusion list (or scrubbing those files) and moving to a Nextcloud app password. |
