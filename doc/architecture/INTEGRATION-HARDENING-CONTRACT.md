...
The next implementation phase should add an on-demand Shopify OAuth-refresh rotate endpoint (`POST /api/shopify/rotate`) and a scheduled rotation-policy check flagging tokens older than `rotation_days`, then add webhook event JSON redaction. Until then, all integration surfaces stay on their current behaviors.