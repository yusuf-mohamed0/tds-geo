...
| --- | --- | --- | --- |
| Shopify | Integration Manager | `shopifyInstall` OAuth writes tokens encrypted into `clients`; `refreshIfExpired` decrypts on read, refreshes via OAuth, re-encrypts on write, and audits to `credential_access_log` (`refresh`); boot hardening encrypts legacy rows and maintains masked vault policy rows (rotation_days=30); admin `POST /api/admin/shopify/rotate` forces on-demand OAuth rotation (`rotate` audit); worker flags due tokens every 5 min; live publish only via explicit `publish:true`, approved/scheduled rows, or `/publish-all` over `approved`. | Hidden draft, manual approval, and scheduled publish semantics remain intact; no raw token JSON in `cms_connections.config`; rotation is manual/admin-only and audited. |
| Ads reporting | Ads Manager | Admin-only route wrapping an external Python reporting runner; PDFs listed from `reports/` and tracked in the `ads_artifact_registry` (generated → approved/rejected → delivered with actor/timestamp audit); internal until approval; no report email sent. | Delivery is recorded manually and never emailed or pushed client-side from the backend; next hardening step is Shopify/connector token rotation behind the credential vault. |
...
| Webhooks / compliance | Integration Manager | Shopify webhook/OAuth HMAC via `SHOPIFY_API_SECRET`; compliance GDPR/uninstall flows; inbound event JSON stored in `activity_logs.metadata` through `redactJson` (PII keys redacted before persistence). | HMAC verification stays mandatory; PII redaction is enforced at every webhook/activity persistence call site via `backend/utils/redact.ts`. |
...
All queued hardening phases (ads registry, Shopify token vault encryption, webhook JSON redaction, Shopify rotate endpoint) are implemented, deployed, and prod-verified. Remaining hardening items are operator-side (vault placeholder re-import, WP-side key provisioning) and documented next steps (vault policy compliance reporting, GSC credential routing through the unified boundary).