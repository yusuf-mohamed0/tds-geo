<?php
/**
 * KOZMO Core Content Sync Service
 *
 * Maps KOZMO Core article data to WordPress posts, handling:
...
class KOZMO_Core_Sync {
    ...
    /**
     * KOZMO Core status → WP post status mapping.
     */
    ...
    /**
     * Create a new WordPress post from KOZMO Core article data.
     *
     * @param array $article KOZMO Core article data. Expected keys:
    ...
    public static function create_post(array $article): array {
        ...
        if (empty($post_data['post_title'])) {
            return [
                'success' => false,
                'message' => __('Post title is required.', 'kozmo-core-integration'),
            ];
        }
        ...
        if (is_wp_error($post_id)) {
            KOZMO_Core_Logger::error('Failed to create post', [
        ...
        // Store KOZMO Core metadata for cross-reference
        update_post_meta($post_id, '_kozmo_core_article_id', $article['kozmo_core_article_id'] ?? '');
        update_post_meta($post_id, '_kozmo_core_imported_at', current_time('mysql'));
        update_post_meta($post_id, '_kozmo_core_source', 'api');

        KOZMO_Core_Logger::info('Post created from KOZMO Core', [
            'post_id'      => $post_id,
            'title'        => $post_data['post_title'],
            'status'       => $post_data['post_status'],
            'kozmo_core_id'    => $article['kozmo_core_article_id'] ?? 'N/A',
        ]);
        ...
        return [
            ...
            'message'  => __('Post created successfully.', 'kozmo-core-integration'),
        ];
    }

    /**
     * Update an existing WordPress post from KOZMO Core article data.
     *
     * @param int   $post_id WordPress post ID.
     * @param array $article KOZMO Core article data (partial or full).
     *
    ...
    public static function update_post(int $post_id, array $article): array {
        ...
        if (is_wp_error($updated)) {
            KOZMO_Core_Logger::error('Failed to update post', [
                'post_id' => $post_id,
        ...
        KOZMO_Core_Logger::info('Post updated from KOZMO Core', [
            'post_id' => $post_id,
        ...
        return [
            'success' => true,
            'post_id' => $post_id,
            'message' => __('Post updated successfully.', 'kozmo-core-integration'),
        ];
    }
    ...
    public static function delete_post(int $post_id, bool $force_delete = false): array {
        ...
        if (!$result) {
            return [
                'success' => false,
                'message' => __('Failed to delete post.', 'kozmo-core-integration'),
            ];
        }

        KOZMO_Core_Logger::info('Post deleted via KOZMO Core', [
            'post_id' => $post_id,
            'force'   => $force_delete,
        ]);
        ...
        return [
            'success' => true,
            'message' => __('Post deleted successfully.', 'kozmo-core-integration'),
        ];
    }

    /**
     * Get a WordPress post by KOZMO Core article ID.
     *
     * @param string $kozmo_core_id The KOZMO Core article UUID.
     *
     * @return WP_Post|null
     */
    public static function get_post_by_kozmo_core_id(string $kozmo_core_id): ?WP_Post {
        $posts = get_posts([
            'meta_key'   => '_kozmo_core_article_id',
            'meta_value' => $kozmo_core_id,
            'post_type'  => 'any',
            'post_status' => 'any',
            'numberposts' => 1,
        ]);
    ...
    /**
     * Build wp_insert_post compatible post data from KOZMO Core article.
     *
     * @param array     $article  KOZMO Core article data.
    ...
    private static function build_post_data(array $article, ?WP_Post $existing = null): array {
        ...
        // Map KOZMO Core status to WP status
    ...
    private static function set_featured_image(int $post_id, string $image_url): void {
        ...
        if (is_wp_error($tmp)) {
            KOZMO_Core_Logger::warning('Failed to download featured image', [
                'post_id'   => $post_id,
                'image_url' => $image_url,
        ...
        if (is_wp_error($attachment_id)) {
            KOZMO_Core_Logger::warning('Failed to sideload featured image', [
                'post_id' => $post_id,
        ...
        KOZMO_Core_Logger::info('Featured image set', [
            'post_id'       => $post_id,
    ...
    private static function set_custom_fields(int $post_id, array $custom_fields): void {
        foreach ($custom_fields as $key => $value) {
            // Skip internal KOZMO Core fields
            if (str_starts_with($key, '_kozmo_core_')) {
                continue;
            }
    ...
    private static function set_seo_meta(int $post_id, array $article): void {
        ...
        // Always store KOZMO Core SEO data
        if (!empty($meta_title)) {
            update_post_meta($post_id, '_kozmo_core_meta_title', $meta_title);
        }
        if (!empty($meta_description)) {
            update_post_meta($post_id, '_kozmo_core_meta_description', $meta_description);
        }
    }
    ...
    /**
     * Get posts that have been imported via KOZMO Core.
     *
     ...
     * @return array
     */
    public static function get_kozmo_core_posts(int $limit = 50, string $status = ''): array {
        $args = [
            'meta_key'   => '_kozmo_core_imported_at',
            'post_type'  => 'any',
