 ...
 * KOZMO Core Logger
 *
 * Provides structured logging for the KOZMO Core WordPress integration.
...
class KOZMO_Core_Logger {
    ...
    private static function log(string $level, string $message, array $context = []): void {
        ...
        $wpdb->insert(
            $wpdb->prefix . KOZMO_CORE_LOG_TABLE,
            [
                'level'      => $level,
                'message'    => $message,
    ...
    public static function get_logs(int $limit = 100, string $level = ''): array {
        global $wpdb;
        $table = $wpdb->prefix . KOZMO_CORE_LOG_TABLE;
