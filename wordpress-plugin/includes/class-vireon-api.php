 ...
 * Vireon REST API
 *
 * Registers custom WP REST API endpoints for Vireon integration.
 * All endpoints are under the `vireon/v1` namespace.
 *
 * Endpoints:
 * - POST /vireon/v1/posts          — Create a new post
 * - PUT  /vireon/v1/posts/{id}    — Update an existing post
 * - DELETE /vireon/v1/posts/{id}  — Delete a post
 * - GET  /vireon/v1/posts/{id}    — Get post details
 * - GET  /vireon/v1/posts         — List posts
 * - POST /vireon/v1/posts/batch   — Batch create/update posts
 * - GET  /vireon/v1/status        — Connection status & health check
 * - POST /vireon/v1/media         — Upload media
 * - GET  /vireon/v1/categories    — List categories (for content mapping)
 * - GET  /vireon/v1/tags          — List tags
 * - GET  /vireon/v1/settings      — Get plugin settings (read-only)
 * - POST /vireon/v1/settings/sync — Force sync configuration
 *
...
class Vireon_API {
    ...
    public static function register_routes(): void {
        $namespace = VIREON_API_NAMESPACE;
    ...
    public static function create_post(WP_REST_Request $request): WP_REST_Response {
        ...
        $result = Vireon_Sync::create_post($article);
    ...
    public static function list_posts(WP_REST_Request $request): WP_REST_Response {
        ...
        // If we have a vireon_article_id query, search by that
        $vireon_id = $request->get_param('vireon_article_id');
        if ($vireon_id) {
            $args['meta_key']   = '_vireon_article_id';
    ...
    /**
     * GET /vireon/v1/posts/{id} — Get a single post.
     */
    ...
    /**
     * PUT /vireon/v1/posts/{id} — Update a post.
     */
    public static function update_post(WP_REST_Request $request): WP_REST_Response {
        ...
        $result = Vireon_Sync::update_post($post_id, $article);
    ...
    public static function delete_post(WP_REST_Request $request): WP_REST_Response {
        ...
        $result = Vireon_Sync::delete_post($post_id, $force_delete);
    ...
    public static function batch_posts(WP_REST_Request $request): WP_REST_Response {
        ...
        foreach ($posts as $index => $article) {
            if (!empty($article['id'])) {
                ...
                $results[] = [
                    'index'   => $index,
                    'action'  => 'update',
                    'result'  => Vireon_Sync::update_post((int) $article['id'], $article),
                ];
            } else {
                // Create new
                $results[] = [
        ...
        Vireon_Logger::info('Batch post operation completed', [
            'total'   => count($results),
    ...
    /**
     * GET /vireon/v1/status — Health check and connection status.
     */
    public static function get_status(): WP_REST_Response {
        ...
        $key_count = (int) $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}vireon_api_keys WHERE is_active = 1"
        );

        // Get Vireon-imported posts count
        $vireon_posts = (int) $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = '_vireon_imported_at'"
        );
    ...
    /**
     * POST /vireon/v1/media — Upload media from URL.
     */
    ...
    /**
     * GET /vireon/v1/categories — List categories.
     */
    ...
    /**
     * GET /vireon/v1/settings — Get plugin configuration (read-only via API).
     */
    public static function get_settings(): WP_REST_Response {
        ...
        return new WP_REST_Response([
            ...
            'data'    => [
                ...
                'seo_plugins'      => self::detect_seo_plugins(),
                'version'          => VIREON_VERSION,
            ],
        ], 200);
    }

    /**
     * GET /vireon/v1/authors — List available authors.
     */
    ...
    /**
     * POST /vireon/v1/webhook — Receive incoming webhook from Vireon.
     */
    public static function handle_webhook(WP_REST_Request $request): WP_REST_Response {
        ...
        if (!empty($settings['webhook_secret'])) {
            $signature = $request->get_header('X-Vireon-Signature');
            ...
            if (!hash_equals($expected, $signature)) {
                Vireon_Logger::warning('Invalid webhook signature received');
                return new WP_REST_Response([
                    'success' => false,
        ...
        $auth = Vireon_Auth::authenticate_request();
        ...
        Vireon_Logger::info('Webhook received', [
            'event' => $event,
            'type'  => gettype($data),
        ]);
        ...
        switch ($event) {
            case 'article.created':
                $result = Vireon_Sync::create_post($data);
                break;

            case 'article.updated':
                $post_id = $data['post_id'] ?? 0;
                if (!$post_id) {
                    // Try to find by vireon_article_id
                    $vireon_id = $data['vireon_article_id'] ?? '';
                    if ($vireon_id) {
                        $post = Vireon_Sync::get_post_by_vireon_id($vireon_id);
                ...
                if (!$post_id) {
                    ...
                    $result = Vireon_Sync::update_post($post_id, $data);
                }
                break;

            case 'article.deleted':
                $post_id = $data['post_id'] ?? 0;
                if (!$post_id && !empty($data['vireon_article_id'])) {
                    $post = Vireon_Sync::get_post_by_vireon_id($data['vireon_article_id']);
                    $post_id = $post ? $post->ID : 0;
                }
                if ($post_id) {
                    $result = Vireon_Sync::delete_post($post_id, true);
                } else {
            ...
            default:
                Vireon_Logger::warning('Unknown webhook event', ['event' => $event]);
                return new WP_REST_Response([
                    'success' => false,
    ...
    private static function format_post(WP_Post $post): array {
        ...
        return [
            ...
            'meta'              => [
                'title'       => get_post_meta($post->ID, '_vireon_meta_title', true),
                'description' => get_post_meta($post->ID, '_vireon_meta_description', true),
            ],
