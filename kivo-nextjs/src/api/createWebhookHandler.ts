...
/**
 * Kivo Geo Next.js Integration - Webhook Route Handler
 *
 * Receives real-time content events from Kivo Geo (article.created,
 ...
 *   import { createWebhookHandler } from '@tds/nextjs-integration/server';
 *
 *   const handler = createWebhookHandler({ apiKey: process.env.KIVO_API_KEY! });
...
export function createWebhookHandler(config: TdsGeoConfig) {
  ...
  /** POST /api/kivo/webhook — Receive content events */
  async function POST(request: NextRequest): Promise<NextResponse<TdsGeoApiResponse>> {
    ...
    try {
      ...
      if (config.debug) {
        console.log('[Kivo Geo] Webhook received:', payload.event);
      }
      ...
      switch (payload.event) {
        ...
        case 'article.updated': {
          ...
          // Try to find existing post by Kivo Geo ID
