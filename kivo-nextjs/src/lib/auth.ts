...
/**
 * Kivo Geo Next.js Integration - API Key Authentication
 *
 * Provides a simple token-based authentication for verifying
 * that incoming requests originate from the Kivo Geo platform.
 */
...
/**
 * Verify that a Next.js API request carries a valid Kivo Geo API key.
 *
 * The key should be sent as a header:
 *   X-Kivo-Key: <your-api-key>
 *
 * Usage in a route handler:
 *   import { verifyRequest } from '@tds/nextjs-integration';
 *
 *   export async function POST(request: NextRequest) {
 *     const auth = verifyRequest(request, process.env.KIVO_API_KEY!);
 *     if (!auth.valid) {
...
export function verifyRequest(request: NextRequest, expectedApiKey: string): AuthResult {
  if (!expectedApiKey) {
    return {
      valid: false,
      reason: 'Kivo Geo API key not configured on server. Set KIVO_API_KEY environment variable.',
    };
  }

  const providedKey = request.headers.get('x-kivo-key');

  if (!providedKey) {
    return {
      valid: false,
      reason: 'Missing X-Kivo-Key header. All Kivo Geo API requests must include this header.',
    };
  }
  ...
  if (!constantTimeCompare(providedKey, expectedApiKey)) {
    return {
      valid: false,
      reason: 'Invalid API key. The X-Kivo-Key value did not match the configured key.',
    };
  }
