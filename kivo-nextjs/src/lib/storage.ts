...
/**
 * Kivo Geo Next.js Integration - Storage Layer
 *
 * Provides a StorageProvider interface and a JSON-file-based implementation
 * that stores Kivo Geo posts as individual JSON files under `.kivo/`.
 *
...
// ══════════════════════════════════════════════════════════════════
// FILE STORAGE — JSON files in .kivo/ directory
// ══════════════════════════════════════════════════════════════════

const DEFAULT_STORAGE_DIR = '.kivo';
...
export class FileStorage implements StorageProvider {
  ...
  async upsertPost(article: TdsGeoArticle): Promise<TdsGeoPost> {
    ...
    // Check if we already have this article (by Kivo Geo ID)
    ...
    const post: TdsGeoPost = {
      ...
      revalidationTags: existing?.revalidationTags || ['kivo'],
      seo: existing?.seo || {
