/**
 * TDS Geo Next.js Integration — API Key Authentication
 *
 * Provides a simple token-based authentication for verifying
 * that incoming requests originate from the TDS Geo platform.
 */
...
/**
 * Verify that a Next.js API request carries a valid TDS Geo API key.
 *
 * The key should be sent as a header:
 *   X-TDS-GEO-Key: <your-api-key>
 *
 * Usage in a route handler:
 *   import { verifyRequest } from '@tds-geo/nextjs-integration';
 *
 *   export async function POST(request: NextRequest) {
 *     const auth = verifyRequest(request, process.env.TDS_GEO_API_KEY!);
 *     if (!auth.valid) {
...
export function verifyRequest(request: NextRequest, expectedApiKey: string): AuthResult {
  if (!expectedApiKey) {
    return {
      valid: false,
      reason: 'TDS Geo API key not configured on server. Set TDS_GEO_API_KEY environment variable.',
    };
  }

  const providedKey = request.headers.get('x-tds-geo-key');

  if (!providedKey) {
    return {
      valid: false,
      reason: 'Missing X-TDS-GEO-Key header. All TDS Geo API requests must include this header.',
    };
  }
  ...
  if (!constantTimeCompare(providedKey, expectedApiKey)) {
    return {
      valid: false,
      reason: 'Invalid API key. The X-TDS-GEO-Key value did not match the configured key.',
    };
  }
