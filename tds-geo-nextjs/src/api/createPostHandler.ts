...
/**
 * TDS Geo Next.js Integration — Single Post Route Handler
 *
 * Factory for GET, PUT, DELETE handlers for `/api/tds-geo/posts/[id]`.
 *
 * Usage in your Next.js App Router:
 *   // app/api/tds-geo/posts/[id]/route.ts
 *   import { createPostHandler } from '@tds-geo/nextjs-integration/server';
 *
 *   const handler = createPostHandler({ apiKey: process.env.TDS_GEO_API_KEY! });
...
export function createPostHandler(config: TdsGeoConfig) {
  ...
  /** Extract the post ID from the URL path (`/api/tds-geo/posts/[id]`) */
  function getPostId(request: NextRequest): string {
    const url = new URL(request.url);
    const segments = url.pathname.split('/').filter(Boolean);
    // Path: ... /api/tds-geo/posts/[id]
    return segments[segments.length - 1] || '';
  }

  /** GET /api/tds-geo/posts/[id] — Get a single post by localId or slug */
  async function GET(
    request: NextRequest,
    ...
    try {
      ...
      // Try by local ID first, then by slug, then by TDS Geo ID
  ...
  /** PUT /api/tds-geo/posts/[id] — Update a post */
  async function PUT(
    request: NextRequest,
    { params }: { params: Promise<{ id: string }> }
    ...
    try {
      ...
      const updatedArticle = {
        ...post,
        ...update,
        id: post.id, // Preserve TDS Geo article ID
  ...
  /** DELETE /api/tds-geo/posts/[id] — Delete a post */
