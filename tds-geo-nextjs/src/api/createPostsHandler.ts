/**
 * TDS Geo Next.js Integration — Posts Route Handler
 *
 * Factory that creates GET (list) and POST (create) handlers for
 * the `/api/tds-geo/posts` endpoint.
 *
 * Usage in your Next.js App Router:
 *   // app/api/tds-geo/posts/route.ts
 *   import { createPostsHandler } from '@tds-geo/nextjs-integration/server';
 *
 *   const handler = createPostsHandler({ apiKey: process.env.TDS_GEO_API_KEY! });
...
export function createPostsHandler(config: TdsGeoConfig) {
  ...
  /** GET /api/tds-geo/posts — List all posts */
  ...
  /** POST /api/tds-geo/posts — Create a new post */
  async function POST(request: NextRequest): Promise<NextResponse<TdsGeoApiResponse>> {
    ...
    try {
      ...
      if (config.debug) {
        console.log('[TDS Geo] Post created:', post.title, `(slug: ${post.slug})`);
      }
