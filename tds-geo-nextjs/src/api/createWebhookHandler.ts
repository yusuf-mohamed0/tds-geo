...
/**
 * TDS Geo Next.js Integration — Webhook Route Handler
 *
 * Receives real-time content events from TDS Geo (article.created,
 ...
 *   import { createWebhookHandler } from '@tds-geo/nextjs-integration/server';
 *
 *   const handler = createWebhookHandler({ apiKey: process.env.TDS_GEO_API_KEY! });
...
export function createWebhookHandler(config: TdsGeoConfig) {
  const storage: StorageProvider = config.storage || new FileStorage();
  const apiKey = config.apiKey;

  /** POST /api/tds-geo/webhook — Receive content events */
  async function POST(request: NextRequest): Promise<NextResponse<TdsGeoApiResponse>> {
    ...
    try {
      const payload = (await request.json()) as TdsGeoWebhookPayload;
      ...
      if (config.debug) {
        console.log('[TDS Geo] Webhook received:', payload.event);
      }

      switch (payload.event) {
        case 'article.created': {
          const article = payload.data as TdsGeoArticle;
        ...
        case 'article.updated': {
          ...
          // Try to find existing post by TDS Geo ID
          const existing = await storage.getPostByTdsGeoId(article.id);

          if (existing) {
            const mergedArticle: TdsGeoArticle = {
              ...article,
          ...
          const newArticle: TdsGeoArticle = {
            ...article,
        ...
        case 'article.deleted': {
          const data = payload.data as { tdsGeoArticleId?: string; postId?: string };
          const tdsGeoId = data.tdsGeoArticleId || data.postId || '';
          ...
          // Find post by TDS Geo ID
          let post = await storage.getPostByTdsGeoId(tdsGeoId);
          if (!post) post = await storage.getPost(tdsGeoId);
          ...
          if (post) {
            ...
            revalidateTdsGeoPost({ localId: post.localId, slug, tags, categories });
          }
