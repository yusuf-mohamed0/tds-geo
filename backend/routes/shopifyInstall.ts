    ...
    try {
      ...
      if (clientId) {
        ...
        if (existingConn.rows.length > 0) {
          await pool.query(
            `UPDATE cms_connections SET endpoint_url = $1, config = $2::jsonb, is_active = true WHERE id = $3`,
            [fullUrl, connConfig, existingConn.rows[0].id]
          );
        } else {
          await pool.query(
            ...
            [clientId, storeName, fullUrl, connConfig]
          );
        }
      }

      // Connect the Shopify connector with the new token
      try {
        const { connectorManager } = await import('../connector-manager');
        const connector = connectorManager.get('shopify');
        if (connector) {
          await connector.connect({ provider: 'shopify', endpointUrl: `https://${shop}`, apiKey: accessToken });
        }
      } catch (connErr) {
        logger.warn('Failed to connect Shopify connector after install', { error: (connErr as Error).message });
      }
