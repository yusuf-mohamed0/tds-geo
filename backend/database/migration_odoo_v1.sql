...
DO $$ BEGIN
  CREATE TYPE odoo_sync_direction AS ENUM ('bidirectional', 'odoo_to_vireon', 'vireon_to_odoo');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE TYPE odoo_conflict_strategy AS ENUM ('vireon_wins', 'odoo_wins', 'manual', 'latest_wins');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
CREATE TABLE IF NOT EXISTS odoo_model_mappings (
  ...
  vireon_entity      VARCHAR(255) NOT NULL,
  ...
  UNIQUE(connection_id, vireon_entity)
);
...
CREATE INDEX IF NOT EXISTS idx_omm_entity ON odoo_model_mappings(vireon_entity);
...
CREATE INDEX IF NOT EXISTS idx_osl_vireon_record ON odoo_sync_log(vireon_record_id);
