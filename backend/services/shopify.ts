...
class ShopifyService {
  ...
  async refreshIfExpired(shopConfig: ShopifyConfig): Promise<ShopifyConfig> {
    if (!this.pool || !shopConfig.shop) return shopConfig;
  ...
  async fetchArticles(shopConfig: ShopifyConfig, blogId: number | null = null, options: {
    ...
    const config = await this.refreshIfExpired(shopConfig);
    const { client, shopName } = this.buildClient(config);
