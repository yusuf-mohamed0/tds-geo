/**
 * Vireon Next.js Integration — API Key Authentication
 *
 * Provides a simple token-based authentication for verifying
 * that incoming requests originate from the Vireon platform.
 */
...
/**
 * Verify that a Next.js API request carries a valid Vireon API key.
 *
 * The key should be sent as a header:
 *   X-Vireon-Key: <your-api-key>
 *
 * Usage in a route handler:
 *   import { verifyRequest } from '@vireon/nextjs-integration';
 *
 *   export async function POST(request: NextRequest) {
 *     const auth = verifyRequest(request, process.env.VIREON_API_KEY!);
 *     if (!auth.valid) {
...
export function verifyRequest(request: NextRequest, expectedApiKey: string): AuthResult {
  if (!expectedApiKey) {
    return {
      valid: false,
      reason: 'Vireon API key not configured on server. Set VIREON_API_KEY environment variable.',
    };
  }

  const providedKey = request.headers.get('x-vireon-key');

  if (!providedKey) {
    return {
      valid: false,
      reason: 'Missing X-Vireon-Key header. All Vireon API requests must include this header.',
    };
  }
  ...
  if (!constantTimeCompare(providedKey, expectedApiKey)) {
    return {
      valid: false,
      reason: 'Invalid API key. The X-Vireon-Key value did not match the configured key.',
    };
  }
