/**
 * Vireon Next.js Integration — Storage Layer
 *
 * Provides a StorageProvider interface and a JSON-file-based implementation
 * that stores Vireon posts as individual JSON files under `.vireon/`.
 *
...
// ══════════════════════════════════════════════════════════════════
// FILE STORAGE — JSON files in .vireon/ directory
// ══════════════════════════════════════════════════════════════════

const DEFAULT_STORAGE_DIR = '.vireon';
...
export class FileStorage implements StorageProvider {
  ...
  async listPosts(options?: {
    ...
    let posts: VireonPost[] = [];

    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
        const post = JSON.parse(data) as VireonPost;
        posts.push(post);
      } catch {
  ...
  async getPost(id: string): Promise<VireonPost | null> {
    try {
      const data = await fs.readFile(this.postPath(id), 'utf-8');
      return JSON.parse(data) as VireonPost;
    } catch {
      return null;
    }
  }

  async getPostBySlug(slug: string): Promise<VireonPost | null> {
    const ids = await this.loadIndex();
    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
  ...
  async getPostByVireonId(vireonId: string): Promise<VireonPost | null> {
    const ids = await this.loadIndex();
    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
        const post = JSON.parse(data) as VireonPost;
        if (post.id === vireonId) return post;
      } catch {
        // Skip
      }
    }
  ...
  async upsertPost(article: VireonArticle): Promise<VireonPost> {
    ...
    // Check if we already have this article (by Vireon ID)
    const existing = await this.getPostByVireonId(article.id);
    const now = new Date().toISOString();

    const post: VireonPost = {
      ...article,
      localId: existing?.localId || crypto.randomUUID(),
      importedAt: existing?.importedAt || now,
      revalidationTags: existing?.revalidationTags || ['vireon'],
      seo: existing?.seo || {
