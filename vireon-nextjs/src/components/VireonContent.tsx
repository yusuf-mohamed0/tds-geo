...
import type { VireonContentProps } from '../types';

/**
 * Vireon Content — Renders Vireon article content with basic styling.
 *
 * Safely renders markdown-or-HTML content into a styled container.
 * Works in both client and server components.
 *
 * @example
 *   import { VireonContent } from '@vireon/nextjs-integration/components';
 *
 *   <VireonContent content={post.content} className="prose" />
 */
export function VireonContent({ content, className = '' }: VireonContentProps) {
  if (!content) {
    return null;
  }
  ...
  if (isHtml) {
    return (
      <div
        className={`vireon-content ${className}`}
  ...
  return (
    <div
      className={`vireon-content vireon-content--markdown ${className}`}
