...
class Auth {
    ...
    public static function init(): void {
        ...
        self::ensure_schema();
        add_action('admin_post_tds_geo_generate_key', [self::class, 'handle_generate_key']);
        add_action('admin_post_tds_geo_revoke_key', [self::class, 'handle_revoke_key']);
    ...
    private static function mask_key(?string $api_key): string {
        ...
        return __('Stored securely', 'tds-geo-wp');
    }
    ...
    public static function validate_key(string $api_key): array {
        ...
        if (null === self::$active_keys) {
            ...
            $results = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT id, api_key, api_key_hash, permissions FROM {$wpdb->prefix}tds_geo_api_keys
    ...
    public static function list_keys(): array {
        ...
        $keys = $wpdb->get_results(
            "SELECT id, api_key, label, permissions, is_active, last_used_at, expires_at, created_at
             FROM {$wpdb->prefix}tds_geo_api_keys ORDER BY created_at DESC",
            ARRAY_A
        );
    ...
    public static function reveal_key(int $key_id): ?string {
        ...
        $row = $wpdb->get_row($wpdb->prepare(
            "SELECT api_key, api_key_encrypted FROM {$wpdb->prefix}tds_geo_api_keys WHERE id = %d",
            $key_id
        ), ARRAY_A);
        if (!$row) return null;
    ...
    public static function handle_generate_key(): void {
        ...
        check_admin_referer('tds_geo_generate_key', 'tds_geo_generate_nonce');
    ...
    public static function handle_revoke_key(): void {
        if (!current_user_can('manage_options')) wp_die('Unauthorized');
        check_admin_referer('tds_geo_revoke_key', 'tds_geo_revoke_nonce');
