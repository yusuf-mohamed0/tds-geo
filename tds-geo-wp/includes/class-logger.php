...
class Logger {
    ...
    private static function log(string $level, string $message, array $context = []): void {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log(sprintf(
                '[Traffic Digital Solutions GEO] %s: %s %s',
                strtoupper($level),
                $message,
    ...
    public static function get_logs(int $limit = 100, string $level = '', string $service = ''): array {
        global $wpdb;
        $table = $wpdb->prefix . 'tds_geo_logs';
        $where = [];
        $params = [];
    ...
    public static function clear(): int {
        global $wpdb;
        return (int) $wpdb->query("TRUNCATE TABLE {$wpdb->prefix}tds_geo_logs");
    }
