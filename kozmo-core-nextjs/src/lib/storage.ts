/**
 * KOZMO Core Next.js Integration — Storage Layer
 *
 * Provides a StorageProvider interface and a JSON-file-based implementation
 * that stores KOZMO Core posts as individual JSON files under `.kozmo-core/`.
 *
...
// ══════════════════════════════════════════════════════════════════
// FILE STORAGE — JSON files in .kozmo-core/ directory
// ══════════════════════════════════════════════════════════════════

const DEFAULT_STORAGE_DIR = '.kozmo-core';
...
export class FileStorage implements StorageProvider {
  ...
  async listPosts(options?: {
    ...
    let posts: KozmoCorePost[] = [];

    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
        const post = JSON.parse(data) as KozmoCorePost;
        posts.push(post);
      } catch {
  ...
  async getPost(id: string): Promise<KozmoCorePost | null> {
    try {
      const data = await fs.readFile(this.postPath(id), 'utf-8');
      return JSON.parse(data) as KozmoCorePost;
    } catch {
      return null;
    }
  }

  async getPostBySlug(slug: string): Promise<KozmoCorePost | null> {
    const ids = await this.loadIndex();
    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
  ...
  async getPostByKozmoCoreId(kozmoCoreId: string): Promise<KozmoCorePost | null> {
    const ids = await this.loadIndex();
    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
        const post = JSON.parse(data) as KozmoCorePost;
        if (post.id === kozmoCoreId) return post;
      } catch {
        // Skip
      }
    }
  ...
  async upsertPost(article: KozmoCoreArticle): Promise<KozmoCorePost> {
    ...
    // Check if we already have this article (by KOZMO Core ID)
    const existing = await this.getPostByKozmoCoreId(article.id);
    const now = new Date().toISOString();

    const post: KozmoCorePost = {
      ...article,
      localId: existing?.localId || crypto.randomUUID(),
      importedAt: existing?.importedAt || now,
      revalidationTags: existing?.revalidationTags || ['kozmo-core'],
      seo: existing?.seo || {
