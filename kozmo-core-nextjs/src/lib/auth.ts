/**
 * KOZMO Core Next.js Integration — API Key Authentication
 *
 * Provides a simple token-based authentication for verifying
 * that incoming requests originate from the KOZMO Core platform.
 */
...
/**
 * Verify that a Next.js API request carries a valid KOZMO Core API key.
 *
 * The key should be sent as a header:
 *   X-KOZMO-Core-Key: <your-api-key>
 *
 * Usage in a route handler:
 *   import { verifyRequest } from '@kozmo-core/nextjs-integration';
 *
 *   export async function POST(request: NextRequest) {
 *     const auth = verifyRequest(request, process.env.KOZMO_CORE_API_KEY!);
 *     if (!auth.valid) {
...
export function verifyRequest(request: NextRequest, expectedApiKey: string): AuthResult {
  if (!expectedApiKey) {
    return {
      valid: false,
      reason: 'KOZMO Core API key not configured on server. Set KOZMO_CORE_API_KEY environment variable.',
    };
  }

  const providedKey = request.headers.get('x-kozmo-core-key');

  if (!providedKey) {
    return {
      valid: false,
      reason: 'Missing X-KOZMO-Core-Key header. All KOZMO Core API requests must include this header.',
    };
  }
  ...
  if (!constantTimeCompare(providedKey, expectedApiKey)) {
    return {
      valid: false,
      reason: 'Invalid API key. The X-KOZMO-Core-Key value did not match the configured key.',
    };
  }
