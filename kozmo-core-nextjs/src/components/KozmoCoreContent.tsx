...
import type { KozmoCoreContentProps } from '../types';

/**
 * KOZMO Core Content — Renders KOZMO Core article content with basic styling.
 *
 * Safely renders markdown-or-HTML content into a styled container.
 * Works in both client and server components.
 *
 * @example
 *   import { KozmoCoreContent } from '@kozmo-core/nextjs-integration/components';
 *
 *   <KozmoCoreContent content={post.content} className="prose" />
 */
export function KozmoCoreContent({ content, className = '' }: KozmoCoreContentProps) {
  if (!content) {
    return null;
  }
  ...
  if (isHtml) {
    return (
      <div
        className={`kozmo-core-content ${className}`}
  ...
  return (
    <div
      className={`kozmo-core-content kozmo-core-content--markdown ${className}`}
