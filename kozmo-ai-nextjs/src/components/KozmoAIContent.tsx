...
import KozmoAIContentProps } from '../types';

/**
 * KozmoAI Content — Renders KozmoAI article content with basic styling.
 *
 * Safely renders markdown-or-HTML content into a styled container.
 * Works in both client and server components.
 *
 * @example
 *   import KozmoAIContent } from '@kozmo-ai/nextjs-integration/components';
 *
 *   <KozmoAIContent content={post.content} className="prose" />
 */
export function KozmoAIContent({ content, className = '' }: KozmoAIContentProps) {
  if (!content) {
    return null;
  }
  ...
  if (isHtml) {
    return (
      <div
        className={`kozmo-ai-content ${className}`}
  ...
  return (
    <div
      className={`kozmo-ai-content kozmo-ai-content--markdown ${className}`}
