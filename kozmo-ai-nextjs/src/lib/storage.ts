/**
 * KozmoAI Next.js Integration — Storage Layer
 *
 * Provides a StorageProvider interface and a JSON-file-based implementation
 * that stores KozmoAI posts as individual JSON files under `.kozmo-ai/`.
 *
...
// ══════════════════════════════════════════════════════════════════
// FILE STORAGE — JSON files in .kozmo-ai/ directory
// ══════════════════════════════════════════════════════════════════

const DEFAULT_STORAGE_DIR = '.kozmo-ai';
...
export class FileStorage implements StorageProvider {
  ...
  async listPosts(options?: {
    ...
    let posts: KozmoAIPost[] = [];

    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
        const post = JSON.parse(data) as KozmoAIPost;
        posts.push(post);
      } catch {
  ...
  async getPost(id: string): Promise<KozmoAIPost | null> {
    try {
      const data = await fs.readFile(this.postPath(id), 'utf-8');
      return JSON.parse(data) as KozmoAIPost;
    } catch {
      return null;
    }
  }

  async getPostBySlug(slug: string): Promise<KozmoAIPost | null> {
    const ids = await this.loadIndex();
    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
  ...
  async getPostByKozmoAIId(kozmo-aiId: string): Promise<KozmoAIPost | null> {
    const ids = await this.loadIndex();
    for (const id of ids) {
      try {
        const data = await fs.readFile(this.postPath(id), 'utf-8');
        const post = JSON.parse(data) as KozmoAIPost;
        if (post.id === kozmo-aiId) return post;
      } catch {
        // Skip
      }
    }
  ...
  async upsertPost(article: KozmoAIArticle): Promise<KozmoAIPost> {
    await this.init();

    // Check if we already have this article (by KozmoAI ID)
    const existing = await this.getPostByKozmoAIId(article.id);
    const now = new Date().toISOString();

    const post: KozmoAIPost = {
      ...article,
      localId: existing?.localId || crypto.randomUUID(),
      importedAt: existing?.importedAt || now,
      revalidationTags: existing?.revalidationTags || ['kozmo-ai'],
      seo: existing?.seo || {
