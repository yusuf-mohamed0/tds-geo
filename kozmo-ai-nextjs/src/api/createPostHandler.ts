/**
 * KozmoAI Next.js Integration — Single Post Route Handler
 *
 * Factory for GET, PUT, DELETE handlers for `/api/kozmo-ai/posts/[id]`.
 *
 * Usage in your Next.js App Router:
 *   // app/api/kozmo-ai/posts/[id]/route.ts
 *   import { createPostHandler } from '@kozmo-ai/nextjs-integration/server';
 *
 *   const handler = createPostHandler({ apiKey: process.env.KOZMO_AI_API_KEY! });
...
export function createPostHandler(config: KozmoAIConfig) {
  const storage: StorageProvider = config.storage || new FileStorage();
  const apiKey = config.apiKey;

  /** Extract the post ID from the URL path (`/api/kozmo-ai/posts/[id]`) */
  function getPostId(request: NextRequest): string {
    const url = new URL(request.url);
    const segments = url.pathname.split('/').filter(Boolean);
    // Path: ... /api/kozmo-ai/posts/[id]
    return segments[segments.length - 1] || '';
  }

  /** GET /api/kozmo-ai/posts/[id] — Get a single post by localId or slug */
  async function GET(
    request: NextRequest,
    { params }: { params: Promise<{ id: string }> }
  ): Promise<NextResponse<KozmoAIAPIResponse<KozmoAIPost>>> {
    try {
      ...
      // Try by local ID first, then by slug, then by KozmoAI ID
      let post = await storage.getPost(identifier);
      if (!post) post = await storage.getPostBySlug(identifier);
      if (!post) post = await storage.getPostByKozmoAIId(identifier);
  ...
  /** PUT /api/kozmo-ai/posts/[id] — Update a post */
  async function PUT(
    request: NextRequest,
    { params }: { params: Promise<{ id: string }> }
  ): Promise<NextResponse<KozmoAIAPIResponse>> {
    ...
    try {
      ...
      const updatedArticle = {
        ...post,
        ...update,
        id: post.id, // Preserve KozmoAI article ID
  ...
  /** DELETE /api/kozmo-ai/posts/[id] — Delete a post */
