# Kivo Geo — Architecture Plan

## Current State Assessment

| Component | Lines | Role | Fate |
|-----------|-------|------|------|
| `backend/` | 15,000+ | Partial Core (basic AI, 25-stage pipeline, Shopify connector) | **Evolve → Kivo Geo** |
| `kivo-wp/` | 6,386 | Fat plugin with full AI, prompts, research, queue, scoring | **Strip → thin connector** |
| `frontend/` | — | Dashboard for backend | **Keep → talks to Core** |
| `kivo-nextjs/` | — | Next.js blog components | **Keep** |
...
**From:** `kivo-wp/includes/class-content-generator.php:432-677`
...
|------|--------|----------|----------|
| `kivo` | `X-Kivo-Key` | `kivo/v1/posts` | This project's Kivo Geo WordPress plugin |
| `kivo` | `X-Kivo-Key` | `kivo/v1/posts` | Legacy Kivo Geo WP plugin |
...
The plugin's REST API (`/kivo/v1/*`) serves as the WordPress-side endpoint that the Core WordPress connector calls. The plugin files to be removed in Phase 3:
